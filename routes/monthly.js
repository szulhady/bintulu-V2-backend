...
router.get("/", (req, res, next) => {
  ...
  var ret = [];
  var dat = [];
  var q = `SELECT MIN(${val}) as min,MAX(${val}) as max, AVG(${val}) as avg,timestamp, MONTH(timestamp) AS month,YEAR(timestamp) AS year, DATE_FORMAT(timestamp, "%d/%c/%y") AS date ,  DATE_FORMAT(timestamp, "%M") AS monthname FROM ${station} WHERE DATE_FORMAT(timestamp, "%Y")="${today}" GROUP BY month,year ORDER BY month;`;
